#!/usr/bin/env python
# coding: utf-8
"""Document Research Assistant for Blog Creation - Streamlit Interface"""
...
class DocumentResearchAgent(Workflow):
    ...
    async def write_report(self, ctx: Context, ev: AnswerEvent) -> ReviewEvent:
        ...
        if not answers:
            logger.warning("No answers collected, cannot generate report")
        ...
        ctx.write_event_to_stream(ProgressEvent(
            progress="Drafting blog post...", 
            step="Writing Draft"
        ))
        logger.info(f"Writing blog post draft with {len(answers)} answers")
        prompt = f"Compose a short blog post (around 100 words) per outline: {outline}\n"
        for ans in prev:
            prompt += f"Q: {ans.question}\nA: {ans.answer}\n"
        resp = await Settings.llm.acomplete(prompt)
        ctx.write_event_to_stream(ProgressEvent(
            progress="Initial blog post draft completed", 
            step="Draft Complete"
        ))
        logger.info("Blog post draft completed")
        logger.info(f"✍️ STEP 3 COMPLETE: write_report - Draft completed")
        return ReviewEvent(report=str(resp))
    ...
    async def review_report(self, ctx: Context, ev: ReviewEvent) -> Union[StopEvent, QuestionEvent]:
        ...
        logger.info(f"Reviewing blog post draft - round {count}")
        prompt = (
            f"Review the blog post for query '{await ctx.get('original_query')}'. "
            "If it adequately answers, return 'OKAY'. Otherwise, list up to 2 concise questions to improve it."
        )
        resp = await Settings.llm.acomplete(prompt)
        text = str(resp)
        if text.strip() == "OKAY" or count >= 3:
            ctx.write_event_to_stream(ProgressEvent(
                progress="Blog post approved!", 
                step="Completed"
            ))
            logger.info("Blog post approved")
            logger.info(f"🔄 STEP 4 COMPLETE: review_report - Content approved")
            return StopEvent(result=ev.report)
        logger.info(f"Blog post needs improvement - generating follow-up questions (round {count})")
...
async def process_document(file_path, query, storage_dir):
    """Process the document and generate a blog post"""
    # Reset progress
    st.session_state.progress_updates = []
    st.session_state.blog_post = ""
    ...
    try:
        ...
        try:
            ...
            st.session_state.blog_post = content
            st.session_state.progress_updates.append({"progress": "Content generated successfully", "step": "Completed"})
            return content
...
async def generate_content_direct(query, doc_tool):
    """Generate content directly without using the workflow"""
    ...
    # STEP 3: Write Report
    logger.info("✍️ STEP 3: Write Report - Drafting study guide content")
    ...
    logger.info(f"✍️ STEP 3 COMPLETE: Write Report - Study guide draft completed")
...
def main():
    ...
    st.markdown("""
    Upload a PDF document and enter a research query to generate a blog post.
    The assistant will analyze your document and create content based on your query.
    """)
    
    # Add explanation of main steps
    with st.expander("How the process works"):
        st.markdown("""
        ### Main Steps of the Content Generation Process:
        
        1. 🔍 **Formulate Questions**: Generate key questions about the topic
        2. 📚 **Answer Questions**: Research answers from the document 
        3. ✍️ **Write Report**: Draft content based on research
    ...
    if st.session_state.uploaded_file_path:
        ...
        # Query input
        st.subheader("2. Enter Research Query")
        query = st.text_input(
            "What would you like to research in this document?",
            placeholder="E.g., 'The importance of budgeting', 'Key findings in the report'",
            help="Enter a topic or question to research in the document",
            disabled=st.session_state.processing
        )
        ...
        if st.button(
            ...
            if query:
                ...
                with tab1:
                    ...
                    st.info("Processing document and generating blog post...")
        ...
        if st.session_state.progress_updates:
            ...
            with tab1:
                ...
                3. ✍️ **Write Report** - Create content from research
                    ...
                    for update in st.session_state.progress_updates:
                        if isinstance(update, dict):
                            ...
                            if "Question Created" in step or "Creating Questions" in step:
                                ...
                                main_steps.append(("✍️ STEP 3: Write Report", progress))
        ...
        if st.session_state.blog_post:
            st.subheader("4. Generated Blog Post")
            st.markdown("---")
            
            # Create a styled container for the blog post
            with st.container():
            ...
            # Allow downloading the blog post
            ...
            with col2:
                st.download_button(
                    ...
                    data=st.session_state.blog_post,
                    file_name="generated_blog_post.txt",
                    mime="text/plain"
                )
